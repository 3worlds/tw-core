...
public class ModelGenerator extends TwCodeGenerator implements JavaCode {
	...
	// all imports generated by codeGenerator
	private Set<String> generatedImports = new HashSet<>();
	// imports listed by user in the rootNode importSnippet property
	private Set<String> userImports = new HashSet<>();
	...
	public ModelGenerator(TreeGraphDataNode root3w, String modelDir) {
		...
		for (int i = 0; i < tblImport.size(); i++) {
			String line = tblImport.getByInt(i);
			line = line.trim();
			if (!line.isBlank())
...
//			// end debug
	...
	public ModelMethodGenerator setMethod(TreeGraphDataNode function) {
		...
		if (hasSpace) {
			generatedImports.add("static " + Distance.class.getCanonicalName() + ".*");
			generatedImports.add(Point.class.getCanonicalName());
			generatedImports.add(Box.class.getCanonicalName());
		}
...
//		boolean hasLifeCycle = hasLifeCycle(focalCats);
		...
		if (method == null) {
			...
			StringTable tblSnippet = (StringTable) function.properties().getPropertyValue(P_FUNCTIONSNIPPET.key());
...
//			if (snippetNode != null) {
//				tblSnippet = (StringTable) snippetNode.properties()
//					.getPropertyValue(P_SNIPPET_JAVACODE.key());
//				snptLines = new ArrayList<>();
//				for (int i = 0; i < tblSnippet.size(); i++)
//					snptLines.add(tblSnippet.getWithFlatIndex(i));
//				snippets.put(mname, snptLines);
//			}
		...
		for (TwFunctionArguments arg : dataStruk.keySet()) {
			...
			if ((!excludedFocalArguments.get(focalLevel).contains(arg)) &&
				...
				for (memberInfo mb : rec.members) {
					if (mb.name != null) {
						if (mb.fullType != null)
							generatedImports.add(mb.fullType);
						String prefix = "";
...
//		if (ftype!=SetInitialState)
		if (!queues.isEmpty()) {
			...
			for (TimerNode q : queues)
				queueNames.add(q.id());
			generatedImports.add(EventQueue.class.getName());
...
//		generatedClassName = packageName + "." + className;
	...
	public String asText(String indent) {
		...
		for (String imp : generatedImports)
			result += "import " + imp + ";\n";
		
		result +=Comments.singleLineComment(Comments.startImportInsertion);
		for (String imp : userImports) 
			result +="import "+ imp + ";\n";
		result += Comments.singleLineComment(Comments.finishImportInsertion);

		if (generatedImports.size() > 0 || userImports.size()>0)
			result += "\n";
