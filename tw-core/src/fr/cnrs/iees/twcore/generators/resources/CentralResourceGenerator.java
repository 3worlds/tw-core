...
/**
 * A utility class to generate core 3worlds components.
 *
 ...
 *
 */
public class CentralResourceGenerator {
...
//=====================================================================================
	private static void generateNodeDoc(Table nodeSheet) {
		...
		output.append("// ").append("generated by ").append(CentralResourceGenerator.class.getSimpleName())
				.append(" on ").append(now).append('\n').append('\n');
...
//=====================================================================================
	private static void generateEnumCode(Table enumSheet, Map<String, String> extraCode,
		...
		// header comments
		String[] moreComments = { "generated by " + CentralResourceGenerator.class.getSimpleName() + " on " + now };
...
//					extraFields[count++] = i;
		...
		// static initialisation block - to record this enum type as a valid property
		// for parsers
		output.append("\tstatic {\n");
		output.append("\t\tValidPropertyTypes.recordPropertyType(").append(propName)
		...
		for (int i = TYPES + 1; i < nodeSheet.getColumnByIndex(PROPTYPE).getCellCount(); i++) {
			if (nodeSheet.getColumnByIndex(PROPTYPE).getCellByIndex(i).getStringValue().equals(propType))
				if (nodeSheet.getColumnByIndex(PROPLIST).getCellByIndex(i).getStringValue().equals("yes")) {
					...
					// header comments
					String[] moreComments = {
							"generated by " + CentralResourceGenerator.class.getSimpleName() + " on " + now };
					...
					output.append("\t\treturn new ").append(propType).append("Set(").append(propType)
							.append(".defaultValue());\n");
					output.append("\t}\n\n");
					// static init block
					output.append("\tstatic {\n");
					output.append("\t\tValidPropertyTypes.recordPropertyType(").append(propType)
							.append("Set.class.getSimpleName(),\n\t\t\t").append(propType)
							.append("Set.class.getName(),defaultValue());\n");
					output.append("\t}\n\n");
...
//=====================================================================================
	 ...
	 * 
	 * @param args none used to generate 3Worlds core components, principally enum
	 *             classes..
	 */
...
//						generateNodeArchetype(sheet);
			...
			for (Table sheet : odf.getTableList()) {
				if (sheet.getTableName().indexOf('.') > 0) {
					Table nodeSheet = odf.getTableByName(sheet.getTableName().split("\\.")[0]);
