...
/**
 * <p>A class to send <em>metadata</em> from {@linkplain DataTracker}s to {@linkplain DataReceiver}s. Metadata
 * are "data about the data", i.e. information that can be useful to the receiver to 
 * process the data (e.g. measurement units, label, range of values...). A DataTracker will
 * send the metadata prior to any data message, assuming its information applies to all those it will
 * send thereafter.</p>
 * <p>In the general case, any data structure can be used as metadata, as long as the data receiver
 * knows how to handle it. DataTrackers implement the {@link Singleton}&lt;{@code M}&gt; interface 
 * for metadata, i.e. they have
 * a {@code getInstance()} method which will return a metadata instance of class {@code M}.</p>
 * <p>In this implementation, we store all metadata in an 
 * {@linkplain ExtendablePropertyList}. As a result, further properties can be added to the metadata
 * after construction -- this to handle complex cases where metadata come from diverse origins.
 * </p>
 * <p>Typical use case for this class:</p>
 * <pre>
 ...
 * </pre>
 * <p>NOTE: although this class is a descendant of {@linkplain OutputData}, its simulator 
 * {@code status} field is set to {@code null}, as this is meaningless for metadata.</p>
 * 
...
// If this had a time field (inherited from OutputData) it would be the startTime of the TimeLine (consistent but redundant)
	...
	/**
	 * This method adds all the properties of the argument to the metadata property list.
