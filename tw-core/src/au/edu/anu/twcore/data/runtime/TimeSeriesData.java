...
/**
 * Data for time series. There may be optimisations later (ie not replicating
 * labels all the time)
 * 
...
public class TimeSeriesData extends TimeData {
	...
	public TimeSeriesData(DataTrackerStatus status, int senderId, int metadataType) {
		super(status, senderId, metadataType);
	}

	public TimeSeriesData(DataTrackerStatus status, int senderId, int metadataType, long time) {
		super(status, senderId, metadataType);
	}
