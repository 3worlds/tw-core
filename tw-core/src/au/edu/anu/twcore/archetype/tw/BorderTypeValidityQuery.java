...
/**
 * A query to check that (1) if edge effec correction = "custom", then
 * a borderType property is provided and
 * (2) borderType dimensions are compatible with the space type and
 * (3) borderType values are valid and
 * (4) 'wrap' borders come in pairs
 * 
...
public class BorderTypeValidityQuery extends Query {
	...
	public Query process(Object input) { // input is a space node
		...
		if (spnode.properties().hasProperty(P_SPACE_EDGEEFFECTS.key())) {
			EdgeEffectCorrection eec = (EdgeEffectCorrection) 
				spnode.properties().getPropertyValue(P_SPACE_EDGEEFFECTS.key());			
			if (eec==EdgeEffectCorrection.custom) {
				...
				if (spnode.properties().hasProperty(P_SPACE_BORDERTYPE.key())) {
					...
					SpaceType stype = (SpaceType) 
						spnode.properties().getPropertyValue(P_SPACETYPE.key());
					int spdim = stype.dimensions();
					StringTable borderTypes = (StringTable) 
						spnode.properties().getPropertyValue(P_SPACE_BORDERTYPE.key());
					if (borderTypes.ndim()==2) {
						if ((borderTypes.size(0)==2)&&        // dim 1 = lower (0) vs. upper (1)
							...
							message = "'borderType' property of class StringTable must have dimension 1 = 2 and dimension 2 = "+spdim;
					} 
					else
				...
				satisfied = true; // valid value of edgeEffect not requiring borderType (NB borderType will be ignored)
		} 
		else
